...
enum LegacySwiftDataImporter {
  ...
  private static func importAll(from context: ModelContext, into dbQueue: DatabaseQueue) throws {
    let instances = try context.fetch(FetchDescriptor<KMReaderSchemaV6.KomgaInstance>())
    let libraries = try context.fetch(FetchDescriptor<KMReaderSchemaV6.KomgaLibrary>())
    let series = try context.fetch(FetchDescriptor<KMReaderSchemaV6.KomgaSeries>())
    let books = try context.fetch(FetchDescriptor<KMReaderSchemaV6.KomgaBook>())
    let collections = try context.fetch(FetchDescriptor<KMReaderSchemaV6.KomgaCollection>())
    let readLists = try context.fetch(FetchDescriptor<KMReaderSchemaV6.KomgaReadList>())
    let customFonts = try context.fetch(FetchDescriptor<KMReaderSchemaV6.CustomFontV1>())
    let pendingProgress = try context.fetch(FetchDescriptor<KMReaderSchemaV6.PendingProgress>())
    let savedFilters = try context.fetch(FetchDescriptor<KMReaderSchemaV6.SavedFilterV1>())
    let epubThemePresets = try context.fetch(FetchDescriptor<KMReaderSchemaV6.EpubThemePresetV1>())

...
- Page Curl indices are local to one coordinator-owned immutable snapshot. UIKit controllers and asynchronous completions must cross update, preload, rotation, and teardown boundaries using stable reader-item identities rather than array indices or view tags. Programmatic page-curl turns keep their `setViewControllers` completion authoritative: `willTransitionTo` also fires for them and must not invalidate the in-flight transition token; a watchdog recovers when UIKit drops the callbacks. When a navigation command already resolves to the current position, clear `navigationTarget` synchronously before committing — committing first lets its observable writes re-enter the adapter within the same SwiftUI flush and loop.
- Scroll reader engine (`ScrollReaderEngine`) restoration resolves anchors strictly by page identity across item-list rebuilds, carrying the pending initial position as a full `ReaderPositionAnchor` (never a bare `ReaderViewItem`). Resolution order: exact item match → remembered split side → focused page → `item.pageID`. Unresolvable anchors must be discarded (return `nil`), never substituted with a positional fallback.
- Cover reader adapters (`NativeCoverPageView`) must never finalize a page-turn transition across an item-list rebuild boundary: `commitTransition` fails closed when the current/target item no longer resolves (discard and re-sync from the committed position), `teardown()` invalidates in-flight transition tokens, and a viewport size change cancels an in-flight user drag.
...
- Split wide pages keep their committed side across layout rebuilds: `ReaderViewModel` remembers the last committed `.first`/`.second` side for the current page (a merge into `.both` preserves it), and position resolution prefers the remembered side. `ReaderPositionAnchor.preferredSplitPart` transports the side through adapter snapshots and rebuild anchors; adapters must propagate it via `ReaderViewItem.preferredSplitPart(preserving:)` whenever they construct a new anchor.
...
- **Dashboard Book Card Style**: Keep Reading renders books as horizontal cards (`BookHorizontalCardView`) by default; classic cover cards remain via the `dashboardHorizontalBookCards` toggle. Pinned read list/collection cards share the same sizing (`LayoutConfig.horizontalCardWidth`/`horizontalCoverWidth`) and text styles (`LayoutConfig.horizontalCard{Title,Secondary,Tertiary}TextStyle(for:)`, one tier up in cozy density only).
- **Advanced Filters**: Search with metadata filters (authors, genres, tags, publishers) using all/any logic.
- **Browse list pagination & ordering**: Browse pages (`SeriesViewModel`/`BookViewModel`/`CollectionsViewModel`/`ReadListsViewModel`) use `PaginationState(pageSize: 50)` infinite scrolling (`onAppear` + `shouldLoadMore(after:)`, last-3 threshold). Online ordering is server-side; offline, collections/read lists fetch all ids once, sort in memory with `sortedByBrowseOrder` (ICU names, pinned-first), and slice pages from the cached id array, while series/books intentionally stay bare SQLite BINARY (server `titleSort` / table size). The app-local pinned flag is invisible to the server, so online pages prepend pinned items and filter them out of the server stream. Sidebar lists read lightweight rows and sort in memory the same way.
- **Metadata ordering**: User-facing metadata lists (authors, publishers, genres, tags, languages) sort with locale-aware ICU collation via `Collection.localizedSorted()` (backed by `localizedStandardCompare`, respecting the app language via `Locale.current`); authors sort by role first, then by localized name inside `Author.sortedByRole()`. Do not revert these to raw `.sorted()` for user-visible metadata ordering. `MetadataIndex` encode keys and SQL clause ordering intentionally keep plain `.sorted()` for determinism.
...
- **Series Batch Read Status**: Series detail book lists support a selection mode (iOS/macOS, hidden offline) for batch Mark Read/Unread via `ReadStatusSelectionToolbar`; Select All covers the whole series via GRDB `fetchAllSeriesBookIds`. Marking loops per-book `BookService.markAsRead/markAsUnread` calls (Komga has no batch endpoint), then resyncs the series and posts `postSeriesBooksDidChange` + `postReadStatusChanged`.
- **Paginated list revalidation**: Notification-driven list refreshes revalidate the loaded page window in place via `PaginationState.replaceItems` (re-fetch `0..<currentPage` as one window, preserving scroll position) — see `BookViewModel.revalidateSeriesBooks`/`revalidateReadListBooks`, `SeriesViewModel.revalidateCollectionSeries`, `DashboardSectionDetailView.revalidateItems`. Pure `.readingProgress` changes skip the fetch unless browse options are progress-sensitive (`isSensitiveToReadingProgress`); Dashboard progress sections always revalidate. Full `pagination.reset()` is reserved for initial loads and explicit user actions.
...
- **Series continue-reading accessory**: Rendered via `tabViewBottomAccessory(isEnabled:)` in `PhoneTabView` on iOS 26.1+ only, driven by the shared `ReadingActionBarContext` from `SeriesDetailView`. The modifier must stay permanently attached with `isEnabled` toggling visibility (conditional attachment rebuilds the tab subtree); the context is cleared at `viewWillDisappear` via `ViewLifecycleObserver`, and publishing is gated by `isReadingBarVisible` so post-teardown tasks no-op. Other platforms use the inline `SeriesReadingActionButton` (`showsInlineReadingAction`); do not reintroduce the old floating `safeAreaInset` bar.
- **Server page placement**: iPhone has no Server tab. The current-server card (`SettingsServerCardView`) plus single-row Management (`SettingsManagementView`) and Account (`SettingsAccountView`) entries live in `SettingsView` on iPhone only; iPad (sidebar) and tvOS keep the full `ServerView` page. Library management (add/edit/scan/delete) stays in `SettingsManagementView` on iPhone and `ServerView` elsewhere.
- **iPhone Library tab**: `TabItem.library` sits between Home and Offline (iOS only); its root is `LibraryBrowseView` → `BrowseView(libraryTab: true)`. The library scope is the global dashboard selection (`dashboard.libraryIds`, edited via the shared `LibraryPickerSheet`; do not add a tab-local store). The leading toolbar button on the Home dashboard, Offline tab, and iPhone Library tab is the shared `LibraryScopeToolbarButton`, which labels the current scope with an explicit `HStack` of icon + `Text` (iOS 26 glass collapses `Label` to icon-only) and is hidden when ≤1 library. Toolbar item content must stay a plain label: the library list, the >1 visibility condition, and the `LibraryPickerSheet` `.sheet` all live on the parent view (loading via `LibraryScopeLoader`), because `.task`/`.onReceive`/`.sheet` inside a `ToolbarItem` are unreliable. The iPhone Browse tab is `BrowseView(searchOnly: true)` with a large `tab.search` navigation title; tvOS keeps the full Browse tab.
- **Per-library browse selection transport**: `NavDestination.browseLibrary` carries its `LibrarySelection` in the destination value, and `NavigationHandlingModifier` injects `\.browseLibrarySelection` from the destination itself (falling back to the ambient sidebar selection) so tab shells without a sidebar propagate it correctly. Do not reintroduce side channels for passing the selection to `BrowseView`.
...
- **Settings page organization**: Top-level groups are Reader / Display / Server (iPhone only) / Behavior / Advanced / About (mirrored in the macOS Settings sidebar). Behavior holds user-facing feature toggles (SSE, System Features, Spotlight); Advanced holds power-user/diagnostic pages (Network, Cache, Logs). OS-level behavior toggles (Handoff, Live Activities, Keep Screen Awake) are collected in the System Features page (`SettingsSystemFeaturesView`) with per-platform sections, not inlined into other pages. Cross-reader toggles belong to that page too, not to the bottom of the Reader section. About is its own page (`SettingsAboutView`), not an inline section. Section entry rows use `SettingsBadgeRow`/`SettingsSectionRow` with a colored icon badge (29×29 rounded square + white SF Symbol); new settings pages must register a `SettingsSection` case with icon and color instead of hand-rolling entry rows.
...
23. **Never render an empty `HStack` or `VStack` — no exceptions**: An `HStack`/`VStack` whose children all collapse still participates in layout (spacing, padding, and size-class behaviors around it shift), so a conditionally empty stack degrades the surrounding layout instead of disappearing. Structure the code so the stack itself is behind the condition (e.g. `if hasContent { HStack { ... } }` or gating at the call site), or extract the optional content so nothing renders at all when there is nothing to show. Do not paper over it with flags, hidden views, or zero-length spacers inside the stack.

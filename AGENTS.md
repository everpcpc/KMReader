...
- **Browse list pagination & ordering**: Series (`SeriesViewModel`), books (`BookViewModel`), collections (`CollectionsViewModel`) and read lists (`ReadListsViewModel`) browse pages all use `PaginationState(pageSize: 50)` infinite scrolling driven by `onAppear` + `shouldLoadMore(after:)` (last-3 threshold). Online, each page is fetched from the server (`syncSeriesPage`/`syncBooksPage`/`syncCollections`/`syncReadLists` with `page`/`size`/`sort`) so ordering is server-side and locale-aware per Komga's rules. Offline, collections/read lists order by locale-aware ICU collation via a one-pass fetch: `fetchAllCollectionIds`/`fetchAllReadListIds` read only the sort-needed lightweight columns and sort in memory with `sortedByBrowseOrder` (shared by `sortCollections`/`sortReadLists`; name branch uses `localizedStandardCompare`, date branches stay numeric) plus pinned-first, and the ViewModel caches the full sorted id array and slices pages from it — sorting happens once per query change instead of once per page, at the cost of snapshot semantics (new items synced mid-scroll appear on next refresh). A `LOCALIZED` collation (backed by `localizedStandardCompare`, registered in `LocalDatabase.open()`'s `prepareDatabase`) remains in the `name` branch of `collectionOrderSQL`/`readListOrderSQL` for the pinned-id queries (`fetchPinnedCollectionIds`/`fetchPinnedReadListIds`). Offline series/books ordering intentionally stays bare SQLite BINARY (`meta_title_sort`/`meta_title`): the series default key is the server-generated `titleSort` value (ICU-sorting it would diverge from online order), and books are a large table where per-page full-table ICU sorting is costly (if ever ICU-ized, use fetch-all-then-sort-once + slice, or a materialized sort key). Pinned-first ordering applies on both paths: offline via `pinnedFirst`; online via a client-side pinned header — `CollectionsViewModel`/`ReadListsViewModel` load pinned ids from local GRDB (`fetchPinnedCollectionIds`/`fetchPinnedReadListIds`, ordered by the current sort) on refresh, prepend them to the first page, and skip pinned ids out of the server-ordered page stream (advancing `serverPage`, which is tracked separately from `pagination.currentPage`; fully-pinned server pages are skipped). The app-local pinned flag (`setCollectionPinned`/`setReadListPinned`) is client-only and invisible to the server, so the server stream must always be filtered against the local pinned set. Sidebar lists are fetched with lightweight rows, not full records: `fetchSidebarCollections` reads only `collection_id/name/dates/is_pinned/series_ids_raw` (member count from the decoded id list) and `fetchSidebarReadLists` reads read-list columns plus a `COUNT` over the `ReadListBookMembership` table instead of decoding `book_ids_raw` (membership is rebuilt on every read-list write path, so the count matches `bookIds.count`); both sort in memory with `sortedByBrowseOrder` + `pinnedFirst`, matching the browse ordering.
...
- **Paginated list revalidation**: Notification-driven list refreshes (projection changes flushed on reader close, SSE syncs, dashboard reload commands) must revalidate the already-loaded page window in place via `PaginationState.replaceItems` — see `BookViewModel.revalidateSeriesBooks` / `revalidateReadListBooks`, `SeriesViewModel.revalidateCollectionSeries`, and `DashboardSectionDetailView.revalidateItems`. They re-fetch pages `0..<currentPage` as one window and keep `currentPage`/`loadID`, so loaded pages and scroll position survive; item rows additionally self-update from GRDB on their own projection notifications. Pure `.readingProgress` changes skip the list fetch entirely unless the current browse options are progress-sensitive (`isSensitiveToReadingProgress`: `.dateRead` sort or read-status filters); Dashboard progress sections (Keep Reading / On Deck / Recently Read) are inherently progress-membership-driven and always revalidate. Full `pagination.reset()` refresh is reserved for initial loads and explicit user actions (refresh button, pull-to-refresh, filter/sort/search changes).
...
- **Series continue-reading accessory**: Rendered by the system via `tabViewBottomAccessory(isEnabled:)` in `PhoneTabView` on iOS 26.1+ only, driven by the shared `ReadingActionBarContext` written from `SeriesDetailView`. The modifier must stay permanently attached with `isEnabled` toggling visibility; conditionally attaching it rebuilds the tab subtree and causes a refresh loop. `SeriesDetailView` clears the context via a `ViewLifecycleObserver` at `viewWillDisappear` time (pop transition start) rather than SwiftUI `onDisappear` (post-teardown), and re-syncs at `viewDidAppear` so a cancelled interactive pop restores the accessory. Publishing is additionally gated by `isReadingBarVisible` (set by the same lifecycle hooks): the resolver's `defer` publish and unstructured projection/refresh tasks can complete after teardown, and must no-op instead of resurrecting the accessory on the Home tab or a pushed book detail page. Platforms without the tab accessory (iOS < 26.1, iPad, macOS, tvOS) show the entry as an inline header button (`SeriesReadingActionButton`, gated by `showsInlineReadingAction`) below the series header block instead; do not reintroduce the old floating `safeAreaInset` bar anywhere.
...
- **iPhone Library tab**: `TabItem.library` sits between Home and Offline (`PhoneTabView` + `OldTabView`, iOS only). Its root is `LibraryBrowseView`: content-first browsing (Apple Books style) — `BrowseView(libraryTab: true)` (no navigation title, no search field, no built-in toolbar library button) is the root view so tab re-tap scrolls to top. The library scope is the **global dashboard selection** (`dashboard.libraryIds`, shared with Home and edited via the shared `LibraryPickerSheet`; do not add a tab-local selection store). The leading toolbar button mirrors the Home library button but labels the current scope (single library name, or the "%lld Libraries" count for multiple/All) — its label must be an explicit `HStack` of icon + `Text` because iOS 26 glass toolbars collapse `Label` to icon-only; hidden when ≤1 library. The Search tab keeps a large `tab.search` navigation title. The iPhone Browse tab is the Search tab instead: `BrowseView(searchOnly: true)` shows a search placeholder until a query is entered; tvOS keeps the full Browse tab.

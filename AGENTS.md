...
- Read list continuation: `ReaderPresentationManager.present` resolves a nil read list context through `ReadListReadingService.ownerContext(forBookId:)`, so a book owned by a read list the user is reading continues in read-list order from any entry point; an explicit context (opened from that read list) always wins. Every non-incognito session with a context records the entry point, including seamless cross-book moves (`updatePresentedBook` records only when the book id changes, not on refreshes of the same book).
...
- Read lists continue like series: only ordered read lists with reading state and a surfaced book take part (a book in progress → surfaced through Keep Reading; at least one finished and an unread book → On Deck, searching forward from the last book read). A surfaced read list owns its books: On Deck drops series suggestions for books owned by a visible read list, so one reading thread appears once. Keep Reading is never merged — Komga already lists every in-progress book, and a locally derived entry there could show a book finished on another device. Resolve on the full read list, then apply the library scope to the resolved entry; never filter the list before resolving (that skips books in hidden libraries). `ReadListReadingSnapshot.mergingOnDeck` is the single merge rule for the On Deck row and its detail page, applied through `OnDeckReadListMerge` (owned by the row's `DashboardSectionViewModel` and by the detail page), which remembers the snapshot it merged and reloads when the published one differs; later pages only filter, never prepend. `ReadListReadingService` owns the published `snapshot` and re-derives it after reading-state writes and on local book-progress/read-list projection changes. Do not make the service post dashboard reloads.

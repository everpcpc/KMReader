...
- Page Curl indices are local to one coordinator-owned immutable snapshot. UIKit controllers and async completions cross update/preload/rotation/teardown boundaries by stable reader-item identity, never array indices or view tags. Programmatic turns keep their `setViewControllers` completion authoritative; `willTransitionTo` also fires for them and must not invalidate the in-flight transition token. Programmatic `setViewControllers` must never land while a pan gesture is active: `willTransitionTo` only fires once a curl actually starts, so coordinators read the page view controller's pan-recognizer state (`isPanGestureActive`) and stash snapshot/refresh/navigation work until the pan ends, exactly like `isTransitioning`; a target on the pan recognizers flushes the stashed work on pan end.
...
- In offline-first reading, `ensureOfflineReady` blocks a next-segment preload while the next book downloads; `ReaderViewModel.nextBookOfflineState` is the single observable for the next book's offline readiness (`.downloading(progress)` while blocked, `.ready` once fully available offline — including when it was already downloaded) and end-page/footer UIs render it (cover/title stay visible, status row below). The row is a permanently reserved, constant-height stack slot (progress bar and ready checkmark share one fixed-height slot, toggled by alpha — never `isHidden`, which would re-lay out the page); in streaming mode (offline-first off) the slot collapses entirely. Adapters refresh end content via the page-presentation invalidation channel; Curl and Webtoon coordinators subscribe to it directly. The next-segment preload trigger distance must stay ahead of the download, not just the page turn.

...
- **iPhone Library tab**: `TabItem.library` sits between Home and Offline (iOS only); its root is `LibraryBrowseView` → `BrowseView(libraryTab: true)`. The library scope is the global dashboard selection (`dashboard.libraryIds`, edited via the shared `LibraryPickerSheet`; do not add a tab-local store). The leading toolbar button on the Home dashboard, Offline tab, and iPhone Library tab is the shared `LibraryScopeToolbarButton`, which labels the current scope with an explicit `HStack` of icon + `Text` (iOS 26 glass collapses `Label` to icon-only) and is hidden when ≤1 library. The iPhone Browse tab is `BrowseView(searchOnly: true)` with a large `tab.search` navigation title; tvOS keeps the full Browse tab.

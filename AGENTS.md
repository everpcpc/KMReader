...
- In offline-first reading, `ensureOfflineReady` blocks a next-segment preload while the next book downloads; `ReaderViewModel.pendingNextBookDownload` is the single observable for that wait and end-page/footer UIs render it (cover/title stay visible, progress line below). Adapters refresh end content via the page-presentation invalidation channel; Curl and Webtoon coordinators subscribe to it directly. The next-segment preload trigger distance must stay ahead of the download, not just the page turn.
